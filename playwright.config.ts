...
// Not 3000. The sibling `shop_sznyt_design` backend lives there,
...
// dev server: static rendering and font loading, which this site's search
...
export default defineConfig({
  ...
  use: {
     ...
     * enough that the next navigation to hang fails with an error pointing at
     * the request instead of the clock.
     */
  ...
  webServer: {
     ...
     *
     * A port that is busy now fails the run outright: a suite that cannot
     * reach its own build should say so rather than grade a stranger. The cost
     * is a build per run.
     */
