 ...
 *
 * This export is for rendering. Asking whether a value *is* the marker stays
 * `isPending`'s job, so the marker's shape is compared in one place.
 */
 ...
 * Once a call site writes `=== DO_UZUPELNIENIA`, changing the marker
 ...
 *
 * The client sent the photographs for every realization before the venues
 * and dates, so both fields ship as `DO_UZUPELNIENIA` until they arrive.
 ...
 * dropped. Each realization module points here instead of repeating this.
 */
 ...
 * that has the page, and a relative URL never resolves there.
 *
 ...
 * suite, which builds and serves locally. In a deployment it would publish
 * canonicals and preview images pointing at the visitor's own machine. Since
 * that fault is invisible in the rendered page
 ...
 * and `/o-nas` spends a screen establishing that the business is two people.
 * Other components that name the numbers point here.
 */
 ...
 *
 * "Telefon - Agnieszka" is what somebody deciding who to call reads,
...
export const site = {
   ...
   * claims a vanity name, so there is nothing to compose a link out of. If
   * the profile ever gains one, this becomes a handle and grows a helper
   * beside `instagramHref`.
   */
