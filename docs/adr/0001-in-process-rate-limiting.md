...
specs asked for exactly this ("per-IP rate limiting held in process memory",
...
how serverless works, and it is not.

## Considered options

**Vercel WAF rate-limit rule.** The only option that genuinely holds: it counts
in one place, applies across every instance, and rejects a flood before it
...
**In process memory.** Chosen. No service, no data store, no money, no
configuration outside the repository, and testable without a network.

## Consequences

**The limit leaks across instances, deliberately.** A deployment running
several instances gives one sender an allowance per instance they land on, and
a redeploy forgets everyone. Accepted: a flood still loses most of its volume,
and the thing being protected is one owner's inbox. Do not "fix" this by
reaching for a data store — revisit the WAF rule instead, and only if real
spam gets through.
...
would have rationed precisely the senders able to rotate it. Vercel always sets
...
tripped control here — is told it was sent. This is the site's worst failure
mode (`…portfolio-spec.md:251`) in miniature, accepted as improbable at this
volume. `[zapytanie] Zgłoszenie odrzucone: limit nadawcy.` in `vercel logs` is
the only evidence it happened.

**It adds a third test seam, against the spec's own rule.** The testing
decisions say "No third seam" (`…portfolio-spec.md:328`), and this introduces
two injected dependencies: the limiter itself, and the clock it reads. The
...
test suite that takes an hour. Recorded rather than assumed: this licenses
injecting long-lived state from `route.ts`, and nothing more.
